...
def writetofile(src, dest, channel_idx, variable_name, src_idx=0, frmt='nc'):
    ...
    with h5py.File(dest, 'a') as f:
        if not 'fields' in f:
            f.create_dataset('fields', shape=(NIMGTOT, 20, fsrc.shape[1], fsrc.shape[2]), dtype='<f4')
    fdest = h5py.File(dest, 'a', driver='mpio', comm=MPI.COMM_WORLD)

    # try: 
    #     fdest['fields']
    # except:
    #     new_dataset = fdest.create_dataset('fields', (NIMGTOT, 20, fsrc.shape[1], fsrc.shape[2]), dtype='<f4')
    #     del new_dataset
